    ...
    <div className="h-full overflow-y-auto">
      ...
      {spaceList?.map((space) => (
        <Link key={space.id} to={`/space/${space.id}`}>
          <div
            className={cn(
              `transition-all mb-2 cursor-pointer inline-flex items-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 h-9 px-4 py-2 w-full justify-start`,
              space.id === spaceId
